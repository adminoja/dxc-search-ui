...
export interface IReportRtpCriminalRecord {
  modal: IDxc_Model_Rtp_CriminalRecord;
}

@Injectable()
export class RtpCriminalRecordMetadata {
  html: string;
  reportName: string;
  reportOfTime: string;
  reportOfDate: string;
  private results: IDxc_Model_Rtp_CriminalRecord[];
  ...
  public getHtml(selectData: IDxc_Model_Rtp_CriminalRecord, title: string) {
      ...
      <div>
        ...
        <div class="ml-3">
          <div class="row">
              ...
              <div class="col-3">
                  <p class="text-success font-20"> ${selectData.idNo}</p>
              </div>
          </div>
          <div class="row">
              <div class="col-3">
              ...
              <div class="col-3">
                  <p class="text-success font-20">${selectData.pname}</p>
              </div>
              <div class="col-3">
                  <p class="text-white font-20">ชื่อ - สกุล</p>
              </div>
              <div class="col-3">
                  <p class="text-success font-20">${selectData.cardFName} ${selectData.cardLName}</p>
              </div>
          </div>
          <div class="row">
              <div class="col-3">
              ...
              <div class="col-3">
                  <p class="text-success font-20">${selectData.caseNoYear}</p>
              </div>
              <div class="col-3">
                  <p class="text-white font-20">สภ. สน. ที่ออกเลขคดีตำรวจ</p>
              </div>
              <div class="col-3">
                  <p class="text-success font-20">${selectData.orgName}</p>
              </div>
          </div>
          <div class="row">
              <div class="col-3">
                  <p class="text-white font-20">วันที่เกิดเหตุ</p>
              </div>
              <div class="col-3">
                  <p class="text-success font-20">${selectData.happendD}</p>
              </div>
              <div class="col-3">
                  <p class="text-white font-20">วันที่จับกุม</p>
              </div>
              <div class="col-3">
                  <p class="text-success font-20">${selectData.chargeD}</p>
              </div>
          </div>
          <div class="row">
              <div class="col-3">
  ...
  public viewgetHtml(selectData: IDxc_Model_Rtp_CriminalRecord, title: string) {
      return this.html = `<html>
          <head>
            <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
            ...
            <script>
              $(document).ready(function() {
                $('#confirmCriminalRecord').click(function(){
                  $('#criminalRecord').modal('toggle'); 
                  $('#criminalRecord').on('hidden.bs.modal', function () {
                ...
                $('#closeCriminalRecord').click(function(){
                  $('#criminalRecord').modal('toggle');
                  $('#criminalRecord').on('hidden.bs.modal', function () {
                      $('body').addClass('modal-open');
                  });  
                });
                $("#criminalRecord").modal({
                  show: false,
                  backdrop: 'static'
                });
                $("#clickCriminalRecord").click(function() {
                $("#criminalRecord").modal("show");             
                });
              });
            </script>
          </head>
          <body>
              ...
              <div class="ml-3">
                  <div class="row">
                      ...
                      <div class="col-3">
                          <p class="text-success font-20"><a class="text-success font-20" colspan="4" id="clickCriminalRecord" type="button">
                          ${selectData.idNo} <i class="material-icons">link</i></a></p>
                      </div>
                  </div>
                  <div class="row">
                      <div class="col-3">
          ...
          <div id="criminalRecord" class="modal fade" tabindex="-1" data-backdrop="static" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content">
                ...
                <div class="modal-body font-20 text-white text-center">
                  ...
                  หมายเลขประจำตัวประชาชนนี้ ${selectData.idNo} ใช่หรือไม่
                </div>
                <div class="modal-footer">
                  <button type="button" id="closeCriminalRecord" class="btn btn-danger">ยกเลิก</button>
                  <a id="confirmCriminalRecord" class="btn btn-primary" target="_blank" href="#/dxcui0014/dxcui0018/%E0%B8%A3%E0%B8%B2%E0%B8%A2%E0%B8%87%E0%B8%B2%E0%B8%99%20Single%20Report/${selectData.idNo}/0/0/0/0/0">ตกลง</a>				
                </div>
              </div>
            </div><!-- /.modal-content -->
          </div><!-- /.modal-dialog -->
  ...
  public getSearchData() {
    ...
    if (localStorage.getItem(schema + 'Results') === '[]' || localStorage.getItem(schema + 'Results') === null) {
      ...
      console.log('rtp-criminal-record' + this.results);
    }
  }
  public getallegationPdf() {
    // const schema = environment.qm.sub[0]['rtp'].sub[0]['criminalrecord'].schema;
    const schema = 'rtp-criminal-record';
    ...
    <span class="list-normal-design-underline departdata">ฐานข้อมูลทะเบียนประวัติอาชญากร</span>
    <span class="list-normal-design-underline count">จำนวน: ${this.count} รายการ
      ...
      for (let i = 0; i < this.results.length; i++) {
        ...
        <span style="text-decoration:underline; font-weight: bold;" class="report-section-source-title">ฐานข้อมูลทะเบียนประวัติอาชญากร</span>

        <div style="padding-left:15px;">
        <strong class="report-field-title">เลขประจำตัวประชาชน	
        </strong>&nbsp;&nbsp;
          <span class="report-field-value" style="text-decoration:underline">
            ${this.changeNull(this.results[i].idNo)}
          </span>&nbsp;&nbsp;
        <strong class="report-field-title">คำนำหน้านาม
        </strong>&nbsp;&nbsp;
          <span class="report-field-value" style="text-decoration:underline">
            ${this.changeNull(this.results[i].pname)}
          </span>&nbsp;&nbsp;
        <strong class="report-field-title">ชื่อ - สกุล
        </strong>&nbsp;&nbsp;
          <span class="report-field-value" style="text-decoration:underline">
            ${this.changeNull(this.results[i].cardFName)}
            ${this.changeNull(this.results[i].cardLName)}
          </span>&nbsp;&nbsp;
        <strong class="report-field-title">เลขคดีตำรวจ
        </strong>&nbsp;&nbsp;
          <span class="report-field-value" style="text-decoration:underline">
            ${this.formatDate(this.results[i].caseNoYear)}
          </span>&nbsp;&nbsp;
        <strong class="report-field-title">ข้อหา
        </strong>&nbsp;&nbsp;
          <span class="report-field-value" style="text-decoration:underline">
            ${this.changeNull(this.results[i].charge)}
          </span>&nbsp;&nbsp;
      </div>
        </div>
   ...
   */
     ...
     */
    public get $results(): IDxc_Model_Rtp_CriminalRecord[] {
      return this.results;
    }
     ...
     */
    public set $results(value: IDxc_Model_Rtp_CriminalRecord[]) {
      this.results = value;
    }
