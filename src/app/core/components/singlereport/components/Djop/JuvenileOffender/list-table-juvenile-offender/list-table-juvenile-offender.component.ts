...
declare var $;
@Component({
  selector: 'app-list-table-juvenile-offender',
  templateUrl: './list-table-juvenile-offender.component.html',
  styleUrls: ['./list-table-juvenile-offender.component.scss']
})
export class ListTableJuvenileOffenderComponent implements OnInit {
  ...
  constructor(private ReportJuvenileOffender: DjopJuvenileOffenderMetadata, public dialog: MatDialog) { }
  ...
  selectRow(Item: any, title: string, departmentImg: string) {
    this.title = title;
    this.img = environment.qm.sub[0]['djop'].sub[0]['juvenileOffender'].img;
    ...
    sessionStorage.setItem('djop-juvenile-offender', JSON.stringify(dataSet));
  ...
  openDialog(arrData) {
    ...
    const dialogRef = this.dialog.open(DialogContentjuvenileOffenderTableDialog, {
      data: {
...
@Component({
  selector: 'dialog-content-juvenile-offender-table-dialog',
  templateUrl: 'dialog-content-juvenile-offender-table-dialog.html',
})
export class DialogContentjuvenileOffenderTableDialog {
  constructor(public dialogRef: MatDialogRef<DialogContentjuvenileOffenderTableDialog>, @Inject(MAT_DIALOG_DATA) public data: any) {
  ...
  open() {
    $(document).ready(function () {
      $("#juvenile-offender-table-full-width-modal").modal("show");
    });
