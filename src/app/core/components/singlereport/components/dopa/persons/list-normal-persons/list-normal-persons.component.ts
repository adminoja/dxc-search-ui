...
import { Dxc_Model_Dopa_Moi_Persons, IDxc_Model_Dopa_Moi_Persons } from 'src/app/core/shared/models/Dxc_Model_Dopa_Moi_Persons';
...
declare var $;
@Component({
  selector: 'app-list-normal-persons',
  templateUrl: './list-normal-persons.component.html',
  styleUrls: ['./list-normal-persons.component.scss'],
  providers: [
    DopaMoiPersonsMetadata
  ]
})
export class ListNormalPersonsComponent implements OnInit {
  moidopapersonResults: IDxc_Model_Dopa_Moi_Persons[];
  moidopapersonCount: number;
  ...
  constructor(private ReportDopaMoiPersons: DopaMoiPersonsMetadata, public dialog: MatDialog) { }

  ngOnInit() {
    this.ReportDopaMoiPersons.getSearchData();
    this.moidopapersonResults = this.ReportDopaMoiPersons.$results;
    this.moidopapersonCount = this.ReportDopaMoiPersons.$count;
    this.report_name = localStorage.getItem('report_name');
    // console.log(this.moidopapersonCount)
  }

  selectRow(Item: any, title: string, departmentImg: string) {
    this.title = title;
    this.img = environment.qm.sub[0]['dopa'].sub[0]['persons'].img;
    this.departmentTitle = environment.qm.sub[0]['dopa'].title;

    const moiDopaPersons = new Dxc_Model_Dopa_Moi_Persons(Item);
    moiDopaPersons.setData(Item);
    const dataSet = moiDopaPersons.getData();
    console.log(dataSet)
    const schema = environment.qm.sub[0]['dopa'].sub[0]['persons'].schema;
    sessionStorage.setItem('dopa-moi-dopa-person', JSON.stringify(dataSet));
    ...
    this.htmlReport = this.ReportDopaMoiPersons.getHtml(this.selectItem, title);
    this.openDialog(this.selectData);
  }
  pdfPreviewlocal() {
    this.reportName = '/ReportDopaMoiPersons';
    localStorage.setItem('dopa-moi-dopa-person', JSON.stringify(this.selectItem));
  }
  ...
  openDialog(arrData) {
    ...
    const dialogRef = this.dialog.open(DialogContentPersonDialog, {
      data: {
  ...
  close() {
    $(document).ready(function () {
      $("#DoppaMoiDopaPerson").modal('toggle');
    });
  }
}

@Component({
  selector: 'dialog-content-person-dialog',
  templateUrl: 'dialog-content-person-dialog.html',
})
export class DialogContentPersonDialog {
  constructor(public dialogRef: MatDialogRef<DialogContentPersonDialog>, @Inject(MAT_DIALOG_DATA) public data: any) {
  ...
  open() {
    $(document).ready(function () {
      $("#person-full-width-modal").modal("show");
    });
