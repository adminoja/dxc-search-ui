...
declare var $;
@Component({
  selector: 'app-list-table-face-photos',
  templateUrl: './list-table-face-photos.component.html',
  styleUrls: ['./list-table-face-photos.component.scss'],
  providers: [
    DopaMoiFacePhotosMetadata
  ]
})
export class ListTableFacePhotosComponent implements OnInit {
  ...
  openDialog(arrData) {
    ...
    const dialogRef = this.dialog.open(DialogContentFacephotosTableDialog, {
      data: {
...
@Component({
  selector: 'dialog-content-face-photo-table-dialog',
  templateUrl: 'dialog-content-face-photo-table-dialog.html',
})
export class DialogContentFacephotosTableDialog {
  constructor(public dialogRef: MatDialogRef<DialogContentFacephotosTableDialog>, @Inject(MAT_DIALOG_DATA) public data: any) {
  ...
  open() {
    $(document).ready(function () {
      $("#face-photo-table-full-width-modal").modal("show");
    });
