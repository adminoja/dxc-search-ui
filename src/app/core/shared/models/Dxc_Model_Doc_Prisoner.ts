...
export interface IDxc_Model_Doc_Prisoner {
  ...
  amountOfTimesBreakDiscipline: string; // จำนวนครั้งที่เคยทำผิดวินัย
  dataId: string; // รหัสข้อมูล
  dataSource: string; // แหล่งข้อมูล
  dataSubmitDate: string; // วันที่ปรับปรุงข้อมูล
  ...
  judgement: string; // ผลของคำสั่งศาล
...
export interface Dxc_Model_Doc_Prisoner {
  addressAmphurText: string; // อำเภอ (ปัจจุบัน)
  addressMooBanText: string; // หมู่บ้าน(ปัจจุบัน)
  addressMooText: string; // หมู่(ปัจจุบัน)
  addressNoText: string; // บ้านเลขที่(ที่อยู่ปัจจุบัน)
...
export class Dxc_Model_Doc_Prisoner {
  docprisoner: any;
  private dataSet: IDxc_Model_Doc_Prisoner;
  constructor(selectData: any) {
    ...
    this.docprisoner = {
      ...
      'amountOfTimesBreakDiscipline': this.changeNull(selectData.amountOfTimesBreakDiscipline), // จำนวนครั้งที่เคยทำผิดวินัย
      'dataId': this.changeNull(selectData.dataId), // รหัสข้อมูล
      'dataSource': this.changeNull(selectData.dataSource), // แหล่งข้อมูล
      'dataSubmitDate': this.changeNull(selectData.dataSubmitDate), // วันที่ปรับปรุงข้อมูล
      'dateOfBirth': this.changeNull(selectData.dateOfBirth), // วันเดือนปีเกิด
      ...
      'judgement': this.changeNull(selectData.judgement), // ผลของคำสั่งศาล
      ...
      'policeCaseId': this.changeNull(selectData.policeCaseId), // เลขคดีตำรวจ
      ...
      'receiveDate': this.changeNull(selectData.receiveDate), // วันที่รับตัว
  ...
  setData(selectData: any) {
    this.docprisoner = {
      ...
      'prisonerId': this.changeNull(selectData.prisonerId), // เลขที่นักโทษ
      'receiveDate': this.changeNull(selectData.receiveDate), // วันที่รับตัว
      'releaseDate': this.formatDate(selectData.releaseDate), // วันที่ปล่อย
      'religious': this.changeNull(selectData.religious), // ศาสนา
      'religiousCode': this.changeNull(selectData.religiousCode), // รหัสศาสนา(จากต้นทาง)
      'sentenceDate': this.formatDate(selectData.sentenceDate), // วันที่ได้รับโทษ
      'sexCode': this.changeNull(selectData.sexCode), // รหัสเพศ(จากต้นทาง)
      'undecidedCaseId': this.changeNull(selectData.undecidedCaseId), // เลขคดีดำ
      'courtName': this.changeNull(selectData.courtName), // เลขคดีดำ
      'person': {
...
<div class="watermark11">
  ...
  /**
   * Getter $dataSet
   * @return {IDxc_Model_Doc_Prisoner}
   */
  public get $dataSet(): IDxc_Model_Doc_Prisoner {
    return this.dataSet;
  }

  /**
   * Setter $dataSet
   * @param {IDxc_Model_Doc_Prisoner} value
   */
  public set $dataSet(value: IDxc_Model_Doc_Prisoner) {
    this.dataSet = value;
  }
}
