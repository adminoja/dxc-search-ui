...
import { moimoacfisheriesillegal } from './moi-moac-fisheries-illegal.interface';
import { IManageService, IManageSingleReport } from '../../manageservice.interface';

@Injectable({
  providedIn: 'root'
})
export class MoiMoacFisheriesIllegalService implements moimoacfisheriesillegal, IManageService, IManageSingleReport {
  apiUrl: string;
  headers: string;
  arrRequestors: any;
  baseUrl: string = environment.Dxc_Model_Moi_Moac_Fisheries_Illegal[0].url.production;

  constructor(private http: HttpClient) {
    this.baseUrl = environment.Dxc_Model_Moi_Moac_Fisheries_Illegal[0].url.production;
  }
  ...
  filters(filter: string, name: string) {
    ...
    return this.http.get(`${this.baseUrl}/${filter}/moac-fisheries-illegal-fishing-cases`, httpOptions);
  }
  ...
  findByCID(id: string, page: number, firstOffset: number, transactionId: string) {
    ...
    return this.http.get(`${this.baseUrl}/${citizenCardNumber}/moac-fisheries-illegal-fishing-cases?transactionId=${transactionId}`, httpOptions);
  }
