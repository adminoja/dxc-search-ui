...
import { Dxc_Model_Cifs_MissingPerson } from '../../../models/Dxc_Model_Cifs_MissingPerson';
import { environment } from 'src/environments/environment';

@Injectable({
  providedIn: 'root'
})
export class MissingPersonService implements IManageService, IManageSingleReport {
  baseUrl: string
  constructor(private http: HttpClient) {
    this.baseUrl = environment.Dxc_Model_Cifs_MissingPerson[0].url.production;
  }
  ...
  findByCID(id: string, page: number, firstOffset: number,transactionId: string) {
    ...
    return this.http.get(`${this.baseUrl}?idCard=${citizenCardNumber}&transactionId=${transactionId}&page=0&size=100&sortDirection=ASC`);
  }
