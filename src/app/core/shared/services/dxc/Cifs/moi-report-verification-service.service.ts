...
export class MoiReportVerificationServiceService implements IManageService, IManageSingleReport {

  baseUrl: string
  constructor(private http: HttpClient) {
    this.baseUrl = environment.Dxc_Model_Cifs_ReportVerification[0].url.production;
  }
  findByCID(id: string, page: number, firstOffset: number,transactionId: string) {
    ...
    return this.http.get(`${this.baseUrl}/${citizenCardNumber}/cifs-report-verification?transactionId=${transactionId}`, httpOptions);
  }
  ...
  filters(filter: string, name: string) {
    ...
    return this.http.get(`${this.baseUrl}/${filter}/cifs-report-verification`, httpOptions);
  }
