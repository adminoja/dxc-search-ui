...
export class CrippleService implements IManageService, IManageSingleReport {

  headers: string;
  arrRequestors: any;
  url: any;
  baseUrl: string = environment.Dxc_Model_Dep_Moi_Cripple[0].url.production;
  ...
  filters(filter: string, name: string, lname?: string[], data?: any, latsName?: string[]) {
    ...
    return this.http.get(`${this.baseUrl}/${filter}/dep-cripple`, httpOptions);
    // return this.http.get('assets/data/dep-cripple.json');
  }
  ...
  findByCID(id: string, page: number, firstOffset: number, transactionId: string) {
    ...
    return this.http.get(`${this.baseUrl}/${citizenCardNumber}/dep-cripple?transactionId=${transactionId}`, httpOptions);
    // return this.http.get('assets/data/dep-cripple.json');
  }
