...
export class ChargeService implements IManageService, IManageSingleReport {
  baseUrl: string
  constructor(private http: HttpClient) { 
    this.baseUrl = environment.Dxc_Model_Agoth_Charge[0].url.production;
  }
  findByCID(id: string, page: number, firstOffset: number,transactionId: string) {
    const accusedId = id;
    return this.http.get(`${this.baseUrl}?accusedId=${accusedId}&transactionId=${transactionId}&page=0&size=100&sortDirection=ASC`);
  }
