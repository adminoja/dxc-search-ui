...
export class LandandcondominiumtitlesService implements IManageService , IManageSingleReport {
  headers: string;
  apiUrl: string = environment.Dxc_Model_Dol_Land_and_Condominium_Titles[0].url.production;
  baseUrl: string = environment.Dxc_Model_Dol_Land_and_Condominium_Titles[0].url.production;

  constructor(private http: HttpClient) {
    this.apiUrl = environment.Dxc_Model_Dol_Land_and_Condominium_Titles[0].url.production;
  }
  filters(filter: string, name: string) {
    ...
    return this.http.get(`${this.baseUrl}/${filter}/moi-dol-land-and-condominium-titles`, httpOptions);
  }
  findByCID(id: string, page: number, firstOffset: number, transactionId: string) {
    ...
    return this.http.get(`${this.baseUrl}/${citizenCardNumber}/moi-dol-land-and-condominium-titles?transactionId=${transactionId}`, httpOptions);
  }
