          ...
          keycloakAuth.init({ onLoad: 'login-required' })
            .success(() => {
              ...
              KeycloakService.auth.logoutUrl =
                keycloakAuth.authServerUrl + "/realms/" + environment.keycloakRealm +
                "/protocol/openid-connect/logout?redirect_uri=" + environment.baseUrl + "/index.html";
      
              resolve(); // ✅ OK เพราะเราใช้ Promise<void>
            })
            .error(() => {
              reject(); // ✅ OK เช่นกัน
            });
